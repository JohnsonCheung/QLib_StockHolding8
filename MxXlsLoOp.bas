Attribute VB_Name = "MxXlsLoOp"
Option Compare Text
Option Explicit
Const CLib$ = "QXls."
Const CMod$ = CLib & "MxXlsLoOp."
...
Function R1Lo&(A As ListObject, Optional InlHdr As Boolean)
If IsLozNoDta(A) Then
   R1Lo = A.ListColumns(1).Range.Row + 1
   Exit Function
End If
R1Lo = A.DataBodyRange.Row - IIf(InlHdr, 1, 0)
End Function

Function R2Lo&(A As ListObject, Optional InlTot As Boolean)
If IsLozNoDta(A) Then
   R2Lo = R1Lo(A)
   Exit Function
End If
R2Lo = A.DataBodyRange.Row + IIf(InlTot, 1, 0)
End Function
...
Private Sub KeepFc__Tst()
KeepFstLc VisLo(SampLo)
End Sub

Private Sub SetLoAutoFit__Tst()
Dim Ws As Worksheet: Set Ws = NwWs
...
SetLoAutoFit LozWsDta(Ws)
ClsCWsNoSav Ws
End Sub

Private Sub BrwLo__Tst()
BrwLo SampLo
Stop
End Sub

Private Sub PtzLo__Tst()
