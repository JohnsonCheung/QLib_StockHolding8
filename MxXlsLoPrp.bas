Attribute VB_Name = "MxXlsLoPrp"
Option Explicit
Option Compare Text
Const CLib$ = "QXls."
Const CMod$ = CLib & "MxXlsLoPrp."
Function Lc(Lo As ListObject, C) As ListColumn:   Set Lc = Lo.ListColumns(C): End Function
Function LcDta(Lo As ListObject, C) As Range
Dim O As Range: Set O = Lc(Lo, C).DataBodyRange '<= If no data, it returns nothing
If IsNothing(O) Then
    Set O = RgRC(Lc(Lo, C).Range, 2, 1)
End If
Set LcDta = O
End Function

Function EntLc(L As ListObject, C) As Range
Set EntLc = ColRgzLc(L, C).EntireColumn
End Function
...
WsnzLo = WszLo(A).Name
End Function
Function LcHdrCell(L As ListObject, C$) As Range
Set LcHdrCell = Lc(L, C).Range(1, 1)
End Function
