Attribute VB_Name = "MxVbStrBrk"
Option Compare Text
Option Explicit
Const CLib$ = "QVb."
Const CNs$ = "Str"
Const CMod$ = CLib & "MxVbStrBrk."
...
Dim P&: P = InStr(S, Sep)
If P = 0 Then Brk1 = S12Trim(S, "", NoTrim): Exit Function
...
If P = 0 Then Brk1Rev = S12Trim(S, "", NoTrim): Exit Function
...
Sub AsgBrk(O1, O2, _
    S, Sep$, Optional NoTrim As Boolean)
...
S1 = Left(S, P - 1)
S2 = Mid(S, P + Len(Sep))
BrkAtSep = S12Trim(S1, S2, NoTrim)
End Function

Function Brk1At(S, P&, Sep, NoTrim As Boolean) As S12
If P = 0 Then
    Brk1At = S12Trim(S, "", NoTrim)
Else
