Attribute VB_Name = "MxDaoDbCcm"
Option Compare Text
Option Explicit
Const CNs$ = "Ccm"
Const CLib$ = "QDao."
Const CMod$ = CLib & "MxDaoDbCcm."
...
Select Case True
Case TarFb = "":            Chk3 = FmtMsgNap("[CcmTbl] in [Db] should have 'Des' which is TarFb, but this TarFb is blank", CcmTbl, D.Name)
'Case NotHasFfn(TarFb):    Chk3 = FmtMsgNap("[CcmTbl] in [Db] should have [Des] which is TarFb, but this TarFb does not Has", CcmTbl, D.Name, TarFb)
Case Not HasFbt(TarFb, RmvFstChr(CcmTbl)):
    Chk3 = FmtMsgNap("[CcmTbl] in [Db] should have [Des] which is TarFb, but this TarFb does not Has [Tbl-RmvFstChr(CcmTbl)]", CcmTbl, D.Name, TarFb, RmvFstChr(CcmTbl))
End Select
End Function
...
For Each I In CcmTny
    ...
    LnkFbt D, RmvFstChr(CcmTbl), TarFb, CcmTbl
Next
End Sub


Private Sub CcmTny__Tst()
Dim D As Database
'
Set D = RelCstPgmDb
