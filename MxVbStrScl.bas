Attribute VB_Name = "MxVbStrScl"
Option Compare Text
Option Explicit
Const CLib$ = "QVb."
Const CNs$ = "Str.Ln"
Const CMod$ = CLib & "MxVbStrScl."
...
Dim O$()
    If Si(Er1) > 0 Then
        O = FmtMsgNap("There are [invalid-SclNy] in given [scl] under these [valid-SclNy].", "Er Ny", JnSpc(Er1), A, JnSpc(Ny))
    End If
    If Si(Er2) > 0 Then
        PushAy O, FmtMsgNap("[Itm] of [Scl] has [valid-SclNy], but it is not one of SclNy nor it has '='", "Er Scl Valid-SclNy", Er2, A, Ny)
    End If
ChkSclNN = O
End Function
...
Function ShfVy(OLin, Lblss$) As Variant()
